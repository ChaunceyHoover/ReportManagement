...
app.config(function($routeProvider, $locationProvider) {
	...
	$routeProvider
		...
		.when('/activate', {
			templateUrl: '/spa/activate-site.html',
			controller: 'ActivateSiteCtrl',
			title: 'Activate Site'
		})
		.when('/disable', {
			templateUrl: '/spa/disabled-sites.html',
			controller: 'DisabledSitesCtrl',
			title: 'Disabled Sites'
		})
		.when('/logs', {
			templateUrl: '/spa/logs.html',
			controller: 'LogsCtrl',
			title: 'Logs'
		})
		.when('/reactivate/:formId', {
			templateUrl: '/spa/reactivate.html',
		...
		.when('/notifications', {
			templateUrl: '/spa/notifications.html',
			controller: 'NotificationCtrl',
			title: 'Notifications'
		})
		.when('/adjustments', {
			templateUrl: '/spa/adjustments.html',
			controller: 'AdjustmentsCtrl',
			title: 'Adjustments'
		})
		.when('/tickets', {
			templateUrl: '/spa/tickets.html',
...
app.controller('IndexCtrl', function($scope, $filter, $window, $location, $mdMenu, $mdSidenav, $mdDialog, $mdToast,
	...
	PermissionsService.getLinks().then(function(response) {
		if (response.data.status_code == 0) { // plot twist: it's always 0
			...
			$scope.supportNumber = response.data.supportNumber;
			$scope.supportNumber = $scope.supportNumber.length == 0 ? "" 
				: "(" + $scope.supportNumber.substr(0, 3) + ") " + $scope.supportNumber.substr(3, 3) + "-" + $scope.supportNumber.substr(6);
			updatePermissions();
		}
	});
...
app.controller('SiteViewCtrl', function($scope, $window, $location, $routeParams, $mdToast, $mdDialog,
	...
	$scope.saveInfo = function() {
		...
		for (var i = 0; i < siteKeys.length; i++) {
			var key = siteKeys[i];
			if ($scope.site[key].compareTo != null) {
				if ($scope.site[key].compareTo(originalSite[key]) !== 0)
					siteChanged = true;
				break;
			} else if ($scope.site[key] != originalSite[key]) {
				siteChanged = true;
